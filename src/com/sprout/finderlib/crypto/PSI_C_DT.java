...
public class PSI_C_DT extends AbstractPSIProtocol<Entry<String,String>, Void, List<String>> {
  //Debugging
  private final String TAG = "PSI_C_DT";
  private final boolean D = true;
  
  
  public PSI_C_DT(CommunicationService s, boolean client) {
    super("PSI_C_DT", s, client);
  }
  
  public PSI_C_DT(String testName, CommunicationService s, boolean client) {
  ...
  protected List<String> conductClientTest(CommunicationService s, Entry<String,String>... input){
    ...
    for( Entry<BigInteger, String> tsj : tsjs ) {
      if(tcis.contains(tsj.getKey())) { 
        // TODO: kj = KDF(tci) s.t zi = tsj
        // TODO: D(tsj.getValue());
        res.add(tsj.getValue());
        count++;
      }
    }
    res.add(0, String.valueOf(count));
